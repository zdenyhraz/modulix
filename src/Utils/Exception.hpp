...
#ifdef DEVELOP
#  ifdef HAS_STACKTRACE
  Exception(const std::string& message, const std::source_location& source, std::stacktrace&& stacktrace = std::stacktrace::current()) :
...
#  else
  Exception(const std::string& message, const std::source_location& source) :
    mMessage(fmt::format("{} [{}:{}]", message, std::filesystem::relative(std::filesystem::path(source.file_name()), GetProjectPath("src")).string(), source.line()))
  {
  }
#  endif
#else
  Exception(const std::string& message, const std::source_location& source) : mMessage(message) {}
#endif
