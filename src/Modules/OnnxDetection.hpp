...
class OnnxDetection : public Module {
  ...
  void Load() override {
    model = OnnxModel(GetProjectPath(GetParameter<std::string>("model path")),
                      "onnx_detection_perf", {"input"},
                      {"boxes", "labels", "scores"});
  }
  ...
  void Process() override {
    ...
    for (const auto &det : detections) {
      LOG_DEBUG("Detection: {}, score: {}, box: [{}, {}, {}, {}]", det.label,
                det.score, det.box.x, det.box.y, det.box.width, det.box.height);
  ...
  ParseModelOutput(const std::vector<Ort::Value> &outputTensor) {
    if (outputTensor.size() != 3)
      throw EXCEPTION("Model output size mismatch (expected 3, got {})",
                      outputTensor.size());
    size_t detectionCount =
        outputTensor[0].GetTensorTypeAndShapeInfo().GetShape()[0];
    ...
    for (size_t idx = 0; idx < detectionCount; ++idx)
      if (scores[idx] > scoreThreshold) {
        cv::Rect box(boxes[idx * 4 + 0], boxes[idx * 4 + 1],
                     boxes[idx * 4 + 2] - boxes[idx * 4],
                     boxes[idx * 4 + 3] - boxes[idx * 4 + 1]);
