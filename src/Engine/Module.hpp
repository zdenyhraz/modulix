...
public:
  ...
  struct Connection
  {
    ...
    std::string GetString() const { return fmt::format("{}:{} -> {}:{}", outputModule->GetName(), outputParameter->GetName(), inputModule->GetName(), inputParameter->GetName()); }
  };
...
public:
  ...
  void SetName(const std::string& name) { moduleName = name; }
